<script lang="ts">
	...
	export let onCreateSubmit: (values: Record<string, unknown>) => Promise<{ success: boolean; data?: unknown; error?: string; message?: string; description?: string }> = () => Promise.resolve({ success: true });
	export let onUpdateSubmit: (id: string | number | null, values: Record<string, unknown>) => Promise<{ success: boolean; data?: unknown; error?: string }> = () => Promise.resolve({ success: true });
	export let onDeleteSubmit: (id: string | number | null) => Promise<{ success: boolean; data?: unknown; error?: string }> = () => Promise.resolve({ success: true });
...
<Table.Root>
	...
	<Table.Body>
		{#each data as row (row.id)}
			<Table.Row class={`${selectedRowId === row.id ? "bg-gray-100" : ""}`}  onclick={() => selectedRowId = row.id as null}>
				{#each fields as field}
